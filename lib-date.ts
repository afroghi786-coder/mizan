...
export function dateToFields(date: Date, type: CalType): DateFields {
  const h = date.getHours(), n = date.getMinutes(), s = date.getSeconds();
  if (type === 'jalali') {
...
export function fieldsToDate(
  y: number, m: number, d: number, h: number, n: number, s: number, type: CalType
): Date | null {
  try {
    if (type === 'jalali') { const g = j2g(y, m, d); return new Date(g.y, g.m - 1, g.d, h, n, s); }
    if (type === 'hijri') { const g = h2g(y, m, d); return new Date(g.y, g.m - 1, g.d, h, n, s); }
...
export function isValidDate(y: number, m: number, d: number, type: CalType): boolean {
  ...
  if (type === 'gregorian') { if (y < 1900 || y > 2200) return false; return d >= 1 && d <= 31; }
  if (type === 'jalali') {
...
export function displayDate(input: string | Date | null, type: CalType = 'jalali'): string {
  if (!input) return '';
...
export function parseDateAny(v: string | Date | null | undefined): Date | null {
  ...
  if (gm) return new Date(+gm[1], +gm[2] - 1, +gm[3], +(gm[4] || 0), +(gm[5] || 0), +(gm[6] || 0));
  const jm = s.match(/^(1[34]\d{2})[\/\-.](\d{1,2})[\/\-.](\d{1,2})(?:[T\s]+(\d{1,2}):(\d{1,2})(?::(\d{1,2}))?)?/);
  if (jm) { const g = j2g(+jm[1], +jm[2], +jm[3]); return new Date(g.y, g.m - 1, g.d, +(jm[4] || 0), +(jm[5] || 0), +(jm[6] || 0)); }
  const d = new Date(s);
